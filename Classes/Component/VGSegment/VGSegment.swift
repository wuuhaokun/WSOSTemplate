...
extension VGSegment {
    ...
    public func labelSizeToFit(_ text: String, font: UIFont) -> CGFloat {
    ...
    public func setupTitleLabel(_ title: String, font: UIFont, index: Int, color: UIColor, frame: CGRect) {
    ...
    public func setupIndicator(_ frame: CGRect) {
    ...
    public func reloadView() {
        ...
        for (i, title) in titles.enumerated() {
            ...
            var width = labelSizeToFit(title, font: font) + margin
            if titles.count < 5 {
                let vslue1:Int = Int(bounds.width)
                width = CGFloat((vslue1/titles.count))
            }
            let labelFrame = CGRect(x: labelX, y: labelY , width: CGFloat(width), height: font.lineHeight)
