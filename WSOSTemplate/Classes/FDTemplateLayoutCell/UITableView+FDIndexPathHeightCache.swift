...
extension UITableView {
    ...
    func fd_insertSections(_ sections: IndexSet, with rowAnimation: UITableView.RowAnimation) {
        if fd_indexPathHeightCache.automaticallyInvalidateEnabled {
    ...
    func fd_deleteSections(_ sections: IndexSet, with rowAnimation: UITableView.RowAnimation) {
        if fd_indexPathHeightCache.automaticallyInvalidateEnabled {
            for section in sections {
                fd_indexPathHeightCache.buildSectionsIfNeeded(section)
                fd_indexPathHeightCache.enumerateAllOrientations(using: { heightsBySection in
    ...
    func fd_reloadSections(_ sections: IndexSet, with rowAnimation: UITableView.RowAnimation) {
        if fd_indexPathHeightCache.automaticallyInvalidateEnabled {
            for section in sections {
                fd_indexPathHeightCache.buildSectionsIfNeeded(section)
                fd_indexPathHeightCache.enumerateAllOrientations(using: { heightsBySection in
    ...
    func fd_insertRows(at indexPaths: [IndexPath], with rowAnimation: UITableView.RowAnimation) {
        if fd_indexPathHeightCache.automaticallyInvalidateEnabled {
    ...
    func fd_deleteRows(at indexPaths: [IndexPath], with rowAnimation: UITableView.RowAnimation) {
        if fd_indexPathHeightCache.automaticallyInvalidateEnabled {
            fd_indexPathHeightCache.buildCachesAtIndexPathsIfNeeded(indexPaths)
    ...
    func fd_reloadRows(at indexPaths: [IndexPath], with rowAnimation: UITableView.RowAnimation) {
        if fd_indexPathHeightCache.automaticallyInvalidateEnabled {
            fd_indexPathHeightCache.buildCachesAtIndexPathsIfNeeded(indexPaths)
            for indexPath in indexPaths {
                fd_indexPathHeightCache.enumerateAllOrientations(using: { heightsBySection in
